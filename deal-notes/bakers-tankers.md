# Bakers Tankers / Bakers Bulk / MG Carriers

**RM:** Bronwan Fouche · **Analysed:** 15 September 2026
⏰ **Client meeting Thursday 17 September 2026 for a final answer.**
15 Vereeniging Road, Alrode, Alberton 1451 · contact **Mr Z Essop**, zaahid@bakerstankers.co.za,
011 494 3001 / 078 425 3307 · reg **2008/014516/07** · VAT **4720248410**

## ⛔ THE FINDING: the current motor insurer excludes what this client carries

The OUTsurance schedule records the business as **"Service & repair, Transport, goods"** and prints,
under **Fleet special conditions**:

> "OUTsurance does not provide cover in the following instances:
> …
> **\* when the vehicle transports hazardous materials**"

The client's own goods in transit quote, prepared 14 Sept by Nautical Underwriting Managers off the
broker's submission, describes the subject matter as:

> "General Goods, **Diesel, Petrol, Illuminated Paraffin, Jet Fuel**, FMCG, Scrap Metal and Scrap
> Batteries."

**Diesel, petrol, paraffin and jet fuel are hazardous materials.** On the face of the two documents,
the trucks carrying them are excluded from their own motor policy.

It is not a drafting slip either way, because the SAME OUTsurance schedule sells
**environmental impairment liability R5,000,000 per vehicle** and records
**"Type of substance: Liquid. Containers used: Yes."** So the insurer has priced a pollution risk
on vehicles its fleet conditions say it does not cover.

⛔ **NOTHING ELSE ON THIS ACCOUNT MATTERS UNTIL THIS IS ANSWERED IN WRITING.** Get OUTsurance to
confirm, on the schedule, either that the hazardous materials exclusion does not apply to this fleet,
or that it does. If it does, the client has a 30-vehicle fleet with no own-damage or third-party
cover while loaded, and that is the entire conversation on Thursday.
**Same shape as Twin Trans**, where King Price came in cheaper on a fuel tanker and had dropped
pollution liability.

## ⚠️ The two documents are NOT comparable, and the first read of them is wrong

| | Vehicles | All-in monthly |
|---|---|---|
| **CURRENT OUTsurance `OT89649119`** | **30** | **R75,457.10** |
| **Alpha motor fleet V2** | **183** | **R138,104.10** |

**Alpha is quoting a fleet six times larger**, so R138,104.10 against R75,457.10 is not a price
comparison. Both reconcile.

**OUTsurance, 30 items:** motor fleet vehicle R62,854.24 + motor fleet trailer R1,717.12 =
R64,571.36 + Sasria R10,885.74 = **R75,457.10**. 26 heavy units on FS registrations
(HLH/HJZ/HMJ/HRC/HSH) + 4 on GP registrations.

**Alpha, 183 items, SI R58,720,832** (reconciles exactly):

| Class | Units | Sum insured | Premium | Sasria |
|---|---|---|---|---|
| Heavy commercial | **29** | R24,147,118 | **R64,392.31** | R9,582.06 |
| Trailers > 3,500kg | **119** | R27,206,482 | R22,663.00 | R10,796.08 |
| Private type | 19 | R5,338,232 | R7,842.75 | R49.78 |
| Light commercial | 16 | R2,029,000 | R3,504.76 | R143.29 |
| | **183** | **R58,720,832** | **R98,402.82** | |

Summary: general R190.00 + motor R98,402.83 + liability R5,800.00 + extensions R1,450.00 +
VAPS R1,372.50 + Sasria R14,334.61 + towing R16,554.16 + GIT R0.00 + broadform R0.00 =
**R138,104.10** ✓

### 🎯 On trucks alone, Alpha is CHEAPER

| | Trucks | Premium | Per truck |
|---|---|---|---|
| OUTsurance | 26 | R62,854.24 | **R2,417.47** |
| Alpha | 29 | R64,392.31 | **R2,220.42** |

**Alpha is R197.05 per truck cheaper (−8.2%) and covers three more of them.** The entire R62,647
difference between the two documents is the **115 extra trailers, 16 light commercials and 19
private cars that OUTsurance does not cover at all**.

⛔ **So the first question for Bronwan is not price. It is: what insures the other 153 vehicles
today?** Either there is another motor policy nobody has sent us, or a large part of this fleet is
uninsured. **Do not present any comparison until that is established.**

## 🚨 The pollution cover goes BACKWARDS at Alpha, and that is the decisive cover here

| | Third party | Pollution / clean-up |
|---|---|---|
| **OUTsurance** | Sub-section B **R5,000,000** | **Environmental impairment liability R5,000,000** per vehicle, R91.09/mo, **claims-made, retro 30 Jun 2026** |
| **Alpha** | Sub-section B **R5,000,000** | **Clean up costs (leakage from vehicle) R1,000,000**, 0.240% = R5,800.00 |

Two problems with Alpha's version:

1. **R1,000,000 against R5,000,000.** A fuel spill on a national road is a multi-million rand
   clean-up before anyone counts third-party property damage.
2. **It sits ONLY on the heavy commercial section.** The liability tables for trailers > 3,500kg,
   light commercials and private types each carry sub-section B R5,000,000 and **no clean-up line at
   all**. On a tanker fleet the product is in the **trailer**, not the horse. A spill from a tanker
   trailer may fall outside a clean-up extension written only against the truck.

⚠️ **And OUTsurance's EIL is claims-made with a retroactive date of 30 June 2026**, so it reaches
back six weeks. Anything before that is uninsured for pollution on either document.

## ✅ UPDATE 16 Sept: Envirosure came back with a REAL quote, and it has ALREADY EXPIRED

Bronwan had sent a blank template on the first pass. The actual quote arrived 16 Sept, and it
changes the shape of the whole account. 🚨 **It is dated 14 August 2026, valid until 14 September
2026. Today is 16 September. The client meeting is tomorrow, 17 September. This quote lapsed two
days ago.** Get it reconfirmed or reissued before it goes anywhere near the client.

**Envirosure Underwriting Managers (Pty) Ltd, FSP 38594**, product **Spill Protect Plan, Transport**,
underwritten by **Centriq Insurance Company (FSP 3417)**, ref `20260814/FP/01`, prepared by Adelle
Hartley. ⚠️ **Broker on the quote is printed as "Vaps"**, attention Nadia Pelser, not Cartrack.
Confirm Cartrack's broker code is on it before it is bound, or the commission goes elsewhere.

**Insured Cover: "General Goods, Diesel, Petrol, Illuminated Paraffin, Jet Fuel, FMCG, Scrap Metal
and Scrap Batteries."** Word for word the same list as the CLP goods-in-transit submission. Cover is
**clean-up and rehabilitation costs caused by a pollution release from transported cargo**:
environmental and ecological restoration, 1st and 3rd party clean-up costs, loading and off-loading.
**Claims-made, retroactive date = inception of the Envirosure policy** (a fresh policy, no tail to
lose). Territory **Africa, south of the Equator**.

| Limit | Annual aggregate |
|---|---|
| Transportation of Dangerous Goods | **R5,000,000** |
| Third Party Liability Solution | **R5,000,000** |
| Riot & Strike Solution | R2,500,000 / R1,000,000 |
| Transfer Assist | R250,000 / R50,000 |
| Side Tank Solution | R150,000 |
| **Specified vehicles** | **27** |

**Excess: nil within RSA, nil outside RSA (the Excess Solution), EXCEPT all Third Party Liability
Solution claims carry a flat R10,000** ("Excess Solutions cover will not apply to Third Party
Liability Solution claims").

**Premium: R14,850/mo environmental impairment + R500/mo Hazcall24 fee + R540/mo VAPS, broker fee
R0.00 = R15,890.00/mo = R190,680.00/yr.** Commission to the broker R1,923.75/mo, included in premium.

### 🎯 The vehicle count is the tell: 27, not 30, not 183

**Envirosure specifies 27 vehicles.** OUTsurance's own depot record reads **"Max number of vehicles
27"**, against the 30 scheduled. Alpha quoted **183**. **Working theory: 27 is the real, active
tanker fleet doing hazardous-goods work; OUTsurance's 30 and Alpha's 183 are wider fleet counts
across the group (Bakers Tankers, Bakers Bulk, MG Carriers) that include vehicles never carrying
fuel.** This is not confirmed. **Bronwan's new 108-page OUTsurance schedule (`OT78078997`,
seen only in a WhatsApp screenshot so far, not yet received as a file) may settle it.** Get it
sent to this chat, not just described.

### What this adds to the programme

| | Monthly |
|---|---|
| Motor, OUTsurance (30 vehicles, unresolved pending the new schedule) | R75,457.10 |
| Goods in transit, Santam → New National | R39,212.15 |
| **Spill Protect Plan, Envirosure** | **R15,890.00** |
| **Full programme, once all three are in place** | **R130,559.25** |

Against the current programme of R117,123.77 (motor + GIT only, no clean-up cover at all today),
that is **+R13,435.48/mo** to add pollution cover the client has never had. **This is the honest
frame: the switch is not a saving once clean-up is included, it is buying real cover for the
first time.** Say that plainly, do not let the GIT saving carry the whole conversation.

## 📋 Superseded: the old note calling Envirosure a blank proposal form

`Envirosure Centriq - The Transport Clean-Up Proposal Form.pdf` was a blank template with no client
details, product "The Transport Clean-Up", limits R1,000,000 to R30,000,000. **That product was
never quoted for this client. The real, priced product is "Spill Protect Plan, Transport" above,**
a different product name from the same underwriting manager, at a fixed R5,000,000 dangerous-goods
limit rather than an R1m-R30m range. Do not mix the two up when talking to the client.

## Goods in transit: the one genuine like-for-like on this account

| | |
|---|---|
| **CURRENT Santam Specialist Marine `AMDBLE265448M`** (Durban), broker **Sterling Hamilton Wright** (code 4349453) | **R41,666.67/mo** |
| **CLP / New National Assurance via Nautical Underwriting Managers** (FSP 43437, JP Liebenberg) | **R39,212.15/mo** |
| | **−R2,454.52/mo ≈ −R29,454/yr** |

Santam: estimated annual haulage fee **R50,000,000 at 1.00%** = R434,782.61 ex VAT, **R500,000.00
incl VAT**, minimum and deposit premium 100%, inclusive of strikes risk at 0.05%. Inception
06/07/2026, renewal 01/07/2027, endorsement 1, authorised by Justin Naidoo 14 July 2026.
CLP: annual carry **R50,000,000 at 0.90%** = R450,000/yr = R37,500/mo, plus Sasria on R50m at
0.0034240% = R1,712.15/mo. Broker commission **20%** on GIT, 12% on Sasria.

**The limits match almost exactly:**

| | Santam | CLP |
|---|---|---|
| Any one conveyance | **R1,620,000** | **R1,620,000** ✓ |
| Driver fidelity | **R1,620,000** | **R1,620,000** ✓ |
| Tarpaulins, nets, ropes, chains | R25,000 | R25,000 ✓ |
| Salvage / load recovery | R25,000 | R25,000 ✓ |
| Debris removal | R25,000 | **R30,000** ✓ gain |
| Containers | R200,000 general purpose, **R500,000 tanktainers** | R500,000 shipping containers |
| General excess | 10% min R10,000 | 10% min R10,000 ✓ |
| Hijack / theft excess | 25% min R25,000 | 25% min R25,000 ✓ |

### ✅ CLP's real gain: territory

Santam covers **RSA and to and from Swaziland, Botswana, Mozambique**. CLP covers **RSA, DRC (not
north of Kolwezi), Namibia, Lesotho, Botswana, Angola, Kenya, Zimbabwe, Swaziland, Zambia, Malawi
and Mozambique**, with a North Mozambique insurgency exclusion. **Twelve territories against four.**
⚠️ **Ask the client whether they actually cross those borders.** On Integral Trading the wide cargo
territory turned out to be unused and therefore worthless.

### ⚠️ CLP's catch: an additional excess Santam does not have

CLP adds, on top of the normal deductibles: vehicles conveying **High Risk Cargo with a value over
R1,000,000** attract **an additional theft/hijack deductible of 25% of claim, minimum R25,000,
unless accompanied by an armed escort from a professional security company.** Its own worked example
puts a R1,500,000 hijacked load at **R525,000 of total excess**.

**High Risk Cargo is defined to include "Precious Metals including Copper, Cobalt, Ferro Chrome etc."
and "Solar Panels / Batteries / Inverters / Generators."** The declared subject matter includes
**Scrap Metal and Scrap Batteries**. So on the scrap loads this clause bites, and the saving is
R29,454 a year against R375,000 of additional excess on one R1.5m load. **Get it confirmed in writing
whether scrap metal and scrap batteries are treated as High Risk Cargo.** Fuel is not on the list.

⚠️ **Also on CLP: theft cover on any load over R500,000 requires either live satellite tracking
operational at the time of the claim, or a convoy of no fewer than three vehicles.** A full fuel load
is well over R500,000, so this applies to essentially every trip.

## 🛰️ The Cartrack lead: 148 units, and it is a condition of cover

- **CLP GIT: live satellite tracking, operational at the time of claim, on every load over
  R500,000** (or convoy of 3). That is nearly every load this fleet moves.
- **29 trucks + 119 trailers = 148 units.** Tanker trailers need their own units, because the cargo
  and the theft exposure travel with the trailer, not the horse.
- OUTsurance's depot record shows security as **"Access control, 24 hours, Unspecified"** and
  **"Alarm with armed response, radio link, Unspecified"**. Nothing about vehicle tracking at all.
- **43 accident claims over 10 years** (see below) also makes the driver-scoring and camera case.

## ⚠️ Other findings

- 🚨 **Entity mismatch.** The Santam GIT insures **"Bakers Tankers, Bakers Bulk & MG Carriers
  FTRR&I"**, three entities. **The CLP quote names "Bakers Tankers" alone.** The OUTsurance
  policyholder is **"Bakers Bulk"**, with the debit order off **Bakers Tankers (Pty) Ltd** at HBZ
  Bank. **Replacing the Santam GIT with CLP as quoted would leave Bakers Bulk and MG Carriers
  uninsured.** Same shape as the seven named insureds on Integral Trading.
- 💰 **OUTsurance projects a Business OUTbonus of R307,010.54 payable 07 August 2028**, on the
  current claims ratio, and the schedule confirms "the new Business OUTbonus is applicable on this
  Fleet policy". **Leaving forfeits it.** At R2,454.52/mo of GIT saving that bonus is worth over ten
  years of the saving. **This must be in the room on Thursday, not discovered afterwards.**
  (Note it is a claims-ratio benefit: above 30% nothing is paid.)
- ⚠️ **Claims: 43 dated fleet incidents over 10 years, R1,588,000 disclosed** across the 22 entries
  that carry a value. Worst single loss **R663,192 on 23 July 2022**. By year: 2017 ×1, 2018 ×7,
  2019 ×6, 2020 ×10, 2021 ×11, 2022 ×8. Mostly "Damage, Accidental". **Alpha's quote does not say it
  was rated on this record.** Get that in writing, same gate as CS Continental and Idol.
- ⚠️ **OUTsurance depot: "Max number of vehicles 27" against 30 scheduled.** Same error shape as
  CS Continental (7 against 9).
- ⚠️ **Address disagrees across the documents:** Santam says **15 Nereeniging Road, Alrode,
  Alberton, 1449**; OUTsurance and CLP say **15 Vereeniging Road, Alrode, Alberton, 1451**. Street
  name and postcode both differ.
- ⚠️ **CLP period of insurance reads "12 months commencing on TBA and ending on TBA"** and the
  postal address is "TBA". No inception date is set.
- ⚠️ **CLP loss history reads "No previous claims in the last 3 years as per supplied document"**,
  against an OUTsurance record showing **eight incidents in 2022 alone**. GIT and motor are different
  classes, but the underwriter should be told, or the quote is not firm.
- **OUTsurance motor excess: basic R50,000** per heavy vehicle, third-party liability R6,980,
  window glass R2,000, accessories nil. **Alpha: heavy commercial and trailers > 3,500kg 10% of
  claim minimum R30,000; light commercial and private 5% minimum R5,000.** So Alpha's minimum is
  lower but it is a percentage, and on a R1.5m truck 10% is R150,000 against OUTsurance's flat
  R50,000. **On any substantial loss OUTsurance's excess is far better.**
- OUTsurance per vehicle also carries additional claims preparation costs R10,000 and loss of keys
  R10,000 (R125.05), medical expenses R5,000, Sasria included, Absa Vehicle and Asset Finance
  interest noted. Alpha carries windscreen R1,450 total and towing R16,554.16.
- ⚠️ **Alpha's Sasria does not add up.** The four section figures (R49.78 + R10,796.08 + R143.29 +
  R9,582.06) total **R20,571.21**, but the premium summary shows **R14,334.61**, a difference of
  **R6,236.60**. The total only reconciles using the summary figure. Query it before the price is
  treated as firm.
- OUTsurance policy inception **03 August 2026**, renewal **01 September 2027**, so it is about six
  weeks old. OUTsurance broker of record is **Virgil Niekerk**, an in-house OUTsurance broker
  (niekerkv@out.co.za), the same in-house arrangement as CS Continental's Lara Gray.

## Where this lands

1. **Written answer from OUTsurance on the hazardous materials exclusion.** Everything else waits.
2. **Establish what covers the other 153 vehicles.** Alpha quotes 183, OUTsurance covers 30.
3. **Submit the Envirosure form** so there is a real clean-up number, at a real limit, on Thursday.
4. **GIT: CLP saves R29,454/yr** and adds eight territories, but names one entity instead of three
   and adds a high-risk-cargo excess that bites on the scrap loads. Fixable, worth doing, not
   automatic.
5. **Alpha on trucks is 8.2% cheaper than OUTsurance and covers the whole fleet**, but its pollution
   cover is one fifth of OUTsurance's and sits on the wrong section. **Do not move motor until
   clean-up is solved, either inside Alpha at R5m+ or alongside it at Envirosure.**
6. **The R307,010.54 OUTbonus is the counter-argument to any motor move.** Cost it openly.
