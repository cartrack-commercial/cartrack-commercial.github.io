...
### Commercial Performance Intelligence — REMOVED 22 Sept 2026
- Built 7 Sep 2026 as the twin of the actuary's call-centre pack, applied to the RM division on
  simulated data with a fictional roster, and linked from a **Management** tile on the launcher
  68 minutes later. **Deleted from the repo and the launcher on 22 Sept 2026 on Anne's instruction.**
- **Why it was removed:** every figure and every RM on it was simulated, but the recommendations
  and open-questions tables assigned **real people** as owners (Anne 26 mentions, Bernard 17,
  Brendan 10, plus the accounts executive's name which was scrubbed on 21 Sept). The repo is
  public and the page was linked from the public landing page, so the division's internal
  management plan, its conversion targets and its inefficiency findings were one click from
  `cartrack-commercial.github.io`, with real names against them.
- **If it is ever rebuilt:** owners go in as roles, never names; it does not go on the public
  launcher; and it waits behind the repo being made private. The build recipe, the cohort-rate
  definitions and the go-live plan are in git history at `performance-intelligence/README.md`
  (last present at commit `7b61a77`'s parent) if the approach is wanted again.
- The actuary's original file carries real agent names and performance data → it stays OFF this
  public repo (session upload only).
- ⚠️ **Security finding (7 Sep, while reading the public `cartrack-rm-system` source for vocabulary):** the masterkey passcodes
  (Brendan, Lourie, Bernard, the Anne/Brendan operator code, [redacted]'s key) and a `SEED_DEALS` block with real prospect names and
